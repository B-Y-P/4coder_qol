...
function b32
dynamic_binding_load_from_path(Application_Links *app, Arena *arena, Mapping *mapping, String_Const_u8 full_path){
  b32 result = false;

  String8 message = push_stringf(arena, "loading bindings: %S\n", full_path);
  ...
  if (file != 0){
    String_Const_u8 filename = string_front_of_path(full_path);
    String_Const_u8 data = dump_file_handle(arena, file);
    Config *parsed = def_config_from_text(app, arena, filename, data);
    comp_error(app, config_stringize_errors(app, arena, parsed));
    fclose(file);
          ...
          if (rvalue.type == ConfigRValueType_Compound){
            ...
            for (Config_Get_Result_Node *node = list.first; node != 0; node = node->next){
              ...
              if (!config_compound_string_member(parsed, src, "cmd", 0, &cmd_string)){
                def_config_push_error(arena, parsed, node->result.pos, "Command string is required in binding");
                goto finish_map;
              }

              if (!config_compound_string_member(parsed, src, "key", 1, &key_string)){
                def_config_push_error(arena, parsed, node->result.pos, "Key string is required in binding");
                goto finish_map;
              }

              for (i32 mod_idx = 0; mod_idx < ArrayCount(mod_string); mod_idx += 1){
                String_Const_u8 str = push_stringf(arena, "mod_%i", mod_idx);
              ...
              // NOTE(rjf): Map read in successfully.
              {
                ...
                if (keycode != 0 && command != 0){
                  ...
                  def_config_push_error(arena, parsed, node->result.pos,
